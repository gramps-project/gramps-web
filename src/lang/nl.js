export default {
  'Optionally, enter existing IMAP credentials to enable e-mail notifications required e.g. for user registration.':
    'Voer optioneel bestaande IMAP-referenties in om vereiste e-mailmeldingen in te schakelen, b.v. voor gebruikersregistratie.',
  'Welcome to Gramps Web': 'Welkom bij Gramps Web',
  'Please upload a file in Gramps XML (.gramps) format without media files.':
    'Upload een bestand in Gramps XML (.gramps) -indeling zonder mediabestanden.',
  'If you intend to synchronize an existing Gramps database with Gramps Web, use the Gramps XML (.gramps) format instead.':
    'Als u van plan bent een bestaande Gramps-database te synchroniseren met Gramps Web, gebruik dan het Gramps XML-formaat (.gramps).',
  'A new version of the app is available.':
    'Er is een nieuwe versie van de app beschikbaar.',
  Blog: 'Blog',
  'Change E-mail': 'E-mail aanpassen',
  'Change password': 'Wachtwoord aanpassen',
  Discard: 'Weggooien',
  History: 'Geschiedenis',
  'New password': 'Nieuw wachtwoord',
  'Old password': 'Oud wachtwoord',
  'Recently browsed objects': 'Onlangs doorzochte objecten',
  'Recently changed objects': 'Onlangs gewijzigde objecten',
  Anniversaries: 'Verjaardagen',
  'Create an admin account': 'Maak een beheerdersaccount aan',
  'SMTP host': 'SMTP-host',
  'Optionally, upload existing family tree data.':
    'Upload optioneel bestaande stamboomgegevens.',
  'Need help? Check out ': 'Hulp nodig? Bekijk ',
  Submit: 'Indienen',
  Lists: 'Lijsten',
  'Manage users': 'Gebruikers beheren',
  Menu: 'Menu',
  'New E-mail': 'Nieuwe e-mail',
  'Select language': 'Taal kiezen',
  'Show in tree': 'In boom weergeven',
  'User settings': 'Gebruikersinstellingen',
  Export: 'Exporteren',
  Search: 'Zoeken',
  Navigation: 'Navigatie',
  Global: 'Globaal',
  'Edit coordinates': 'Coördinaten bewerken',
  'Search %s': '%s zoeken',
  'Latest Blog Post': 'Laatste blogbericht',
  'Align the image': 'Afbeelding uitlijnen',
  'Select a point on the map': 'Selecteer een punt op de kaart',
  'Select an existing citation': 'Selecteer een bestaand citaat',
  'Place Hierarchy': 'Plaats-hiërarchie',
  'Keyboard Shortcuts': 'Sneltoetsen',
  'Show this dialog': 'Toon deze dialoog',
  "To start using the blog, add a source with tag 'Blog'.":
    "Voeg een bron toe met label 'Blog' om de blog te gebruiken.",
  'Enter the details for the admin user.':
    'Voer de gegevens in voor de beheerder.',
  'SMTP port': 'SMTP-poort',
  'SMTP user': 'SMTP-gebruiker',
  'SMTP password': 'SMTP-wachtwoord',
  'From address': 'Van-adres',
  'Gramps Web base URL': 'Gramps Web basis-url',
  'Upload family tree': 'Stamboom uploaden',
  'the documentation': 'de documentatie',
  'Unsupported format': 'Niet-ondersteunde opmaak',
  'The Gramps package format (.gpkg) is currently not supported.':
    'Het Gramps-pakketformaat (.gpkg) wordt momenteel niet ondersteund.',
  Username: 'Gebruikersnaam',
  Password: 'Wachtwoord',
  login: 'login',
  'Lost password?': 'Wachtwoord vergeten?',
  'Register new account': 'Nieuw account aanmelden',
  All: 'Alle',
  'Select at least one object type': 'Selecteer ten minste één objecttype',
  Properties: 'Eigenschappen',
  filter: 'filteren',
  'Switch to custom type': 'Schakelen naar aangepast type',
  'Switch to default type': 'Schakelen naar standaardtype',
  'Custom type': 'Aangepast type',
  unconfirmed: 'onbevestigd',
  Guest: 'Gast',
  Member: 'Lid',
  disabled: 'uitgeschakeld',
  Contributor: 'Bijdrager',
  Editor: 'Redacteur',
  Owner: 'Eigenaar',
  'Replace file': 'Bestand vervangen',
  Year: 'Jaar',
  Month: 'Maand',
  Day: 'Dag',
  Administration: 'Administratie',
  'Update search index': 'Zoekindex bijwerken',
  'Manually updating the search index is usually unnecessary, but it may become necessary after an upgrade.':
    'Het handmatig bijwerken van de zoekindex is meestal niet nodig, maar kan nodig zijn na een upgrade.',
  Sort: 'Sortering',
  'Stop editing': 'Bewerking stoppen',
  'Clear all filters': 'Alle filters wissen',
  'Add Tag': 'Label toevoegen',
  'Manage search index': 'Zoekindex beheren',
  'Export user details': 'Gebruikersgegevens exporteren',
  'Add a new user': 'Een nieuwe gebruiker toevoegen',
  'Edit user': 'Gebruiker bewerken',
  'Import user accounts': 'Gebruikersaccounts importeren',
  'Generate a ZIP archive with all media files.':
    'Maak een zip-bestand aan met alle mediabestanden.',
  'Export your family tree': 'Stamboom exporteren',
  'Export your media files': 'Uw mediabestanden exporteren',
  'Error parsing JSON file': 'Fout bij verwerking JSON-bestand',
  'Storing configuration': 'Configuratie opslaan',
  'Importing family tree': 'Stamboom importeren',
  'E-mail settings': 'E-mailinstellingen',
  'Creating owner account': 'Eigenaar-account aanmaken',
  'Copy to clipboard': 'Naar klembord kopiëren',
  'Usage quotas': 'Gebruiksquota',
  'Not authorized': 'Niet geautoriseerd',
  'This e-mail address is already in use': 'Dit e-mailadres is al in gebruik',
  'This user name is already in use': 'Deze gebruikersnaam is al in gebruik',
  'Already have an account?': 'Heeft u al een account?',
  'reset password': 'wachtwoord opnieuw instellen',
  'New account registered successfully.':
    'Nieuw account succesvol geregistreerd.',
  'Please confirm your e-mail address by clicking the link in the e-mail you received and then wait for the tree owner to activate your account.':
    'Bevestig uw e-mailadres door op de link in de e-mail te klikken die u heeft ontvangen en wacht vervolgens tot de eigenaar van de stamboom uw account activeert.',
  'A password reset link has been sent by e-mail.':
    'Een link voor het opnieuw instellen van uw wachtwoord is per e-mail verzonden.',
  'Copy URL': 'URL kopiëren',
  'Import Media Files': 'Mediabestanden importeren',
  'Upload a ZIP archive with files for existing media objects.':
    'Upload een ZIP-archief met bestanden voor bestaande media-objecten.',
  'Delete this object?': 'Dit object verwijderen?',
  'Object %s deleted.': 'Object %s is verwijderd.',
  'Registration link': 'Registratiekoppeling',
  Done: 'Voltooid',
  'Set Status': 'Status instellen',
  'In Progress': 'In afhandeling',
  Open: 'Open',
  Attachments: 'Bijlagen',
  Task: 'Taak',
  Tasks: 'Taken',
  'New Task': 'Nieuwe taak',
  'Show in blog': 'In Blog weergeven',
  'Set Priority': 'Prioriteit instellen',
  'Event Year': 'Jaar gebeurtenis',
  'Common ancestors': 'Gemeenschappelijke voorouders',
  Blocked: 'Geblokkeerd',
  'Edit place type': 'Type plaats bewerken',
  'Edit event type': 'Type gebeurtenis bewerken',
  'Shared DNA': 'Gedeeld DNA',
  'Largest Segment': 'Grootste segment',
  'Text Recognition': 'Tekstherkenning',
  'Chromosome Browser': 'Chromosoom-browser',
  'Shared Segments': 'Gedeelde segmenten',
  'Save as Note': 'Opslaan als Notitie',
  DNA: 'DNA',
  Matches: 'Overeenkomsten',
  'Remove bookmark': 'Bladwijzer verwijderen',
  'Bookmark this': 'Bladwijzer aanmaken',
  'Ancestor frequency': 'Voorouderfrequentie',
  Administrator: 'Administrator',
  '%s media objects with missing file':
    '%s media-objecten met ontbrekend bestand',
  '%s media objects with missing checksum':
    '%s media-objecten met ontbrekend controlegetal',
  'Only places related to events':
    'Alleen plaatsen met betrekking tot evenementen',
  'Media file status': 'Status mediabestand',
  'This tool checks the database for integrity problems, fixing the problems it can.':
    'Dit hulpmiddel controleert de database op integriteitsproblemen en lost deze zo mogelijk op.',
  'The Family Tree you are trying to load is in a schema version not supported by this version of Gramps Web. Therefore you cannot load this Family Tree without upgrading its schema. This action cannot be undone.':
    'De stamboom die u probeert te laden, is in een schemaversie die niet wordt ondersteund door deze versie van Gramps Web. Daarom kunt u deze stamboom niet laden zonder het schema te upgraden. Deze actie kan niet ongedaan worden gemaakt.',
  'Upgrade database': 'Database bijwerken',
  'The Family Tree you are trying to load is in a schema version not supported by this version of Gramps Web. Therefore you cannot load this Family Tree until the tree administrator has upgraded its schema.':
    'De stamboom die u probeert te laden, is in een schemaversie die niet wordt ondersteund door deze versie van Gramps Web. Daarom kunt u deze stamboom niet laden totdat de boombeheerder zijn schema heeft geüpgraded.',
  'Clear the family tree by removing all existing objects. Optionally, select specific types of objects for deletion.':
    'Wis de stamboom door alle bestaande objecten te verwijderen. Selecteer optioneel specifieke soorten objecten om te verwijderen.',
  'Password successfully updated': 'Wachtwoord succesvol bijgewerkt',
  'Wrong username or password': 'Verkeerde gebruikersnaam of wachtwoord',
  'E-mail successfully updated': 'E-mail succesvol bijgewerkt',
  'Danger Zone': 'Gevarenzone',
  'Password is required': 'Wachtwoord is vereist',
  'Delete all objects': 'Alle objecten verwijderen',
  'Please re-enter your password to continue.':
    'Wachtwoord opnieuw invoeren om door te gaan.',
  'Add multiple objects': 'Meerdere objecten toevoegen',
  'Delete Person': 'Persoon verwijderen',
  'Delete Family': 'Familie verwijderen',
  'Delete Place': 'Plaats verwijderen',
  'Delete Source': 'Bron verwijderen',
  'Delete Note': 'Notitie verwijderen',
  'Delete Tag': 'Label verwijderen',
  'Edit Tag': 'Label bewerken',
  'Delete Citation': 'Citaat verwijderen',
  'Delete Repository': 'Opslagplaats verwijderen',
  'Revision History': 'Revisiegeschiedenis',
  'Delete Event': 'Gebeurtenis verwijderen',
  Revisions: 'Revisies',
  'Delete Media': 'Media verwijderen',
  simple: 'eenvoudig',
}
