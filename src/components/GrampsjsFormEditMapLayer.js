...
class GrampsjsFormEditMapLayer extends GrampsjsObjectForm {
  ...
  renderForm () {
    ...
    <grampsjs-map
      latitude="${this.pinCoordinates.length === 2 ? this.pinCoordinates[0] : this._getLatCenter()}"
      longitude="${this.pinCoordinates.length === 2 ? this.pinCoordinates[1] : this._getLongCenter()}"
      zoom="${this._getZoom()}"
      mapid="media-map"
      id="map"
  ...
  _placeImage (lat, long, width) {
    ...
    img.onload = () => {
      ...
      const height = imgHeight / imgWidth * width * Math.cos(lat / 180 * Math.PI)
      this._setBounds([[lat - height, long], [lat, long + width]])
    }
    img.src = url
  }
  ...
  _setTopLeft (lat, long) {
    ...
    const bounds = [[lat - h, long], [lat, long + w]]
    this._setBounds(bounds)
  }
...
window.customElements.define('grampsjs-form-edit-map-layer', GrampsjsFormEditMapLayer)
