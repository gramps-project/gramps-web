...
class GrampsjsFirstRun extends GrampsjsTranslateMixin(LitElement) {
  ...
  render () {
    ...
    : ''}
        ...
        <mwc-textfield @input="${this.checkValidity}" outlined required id="email" label="E-mail" type="email"></mwc-textfield>
        <mwc-textfield @input="${this.checkValidity}" outlined id="full_name" label="Full name" type="text"></mwc-textfield>
    ...
    : ''}
        ...
        <div style="visibility:${(this.stateUser === STATE_DONE) && (this.stateConfig !== STATE_PROGRESS) && (this.stateTree !== STATE_PROGRESS) ? 'visible' : 'hidden'};">
          <mwc-button
            raised
            label="start"
            type="submit"
  ...
  _showProgress (text, status) {
    if (status === STATE_PROGRESS) {
      ...
      ${text}
      <span class="icon">
        <mwc-circular-progress indeterminate density="-7"></mwc-circular-progress>
      </span>
      `
    }
  ...
  checkValidity () {
    ...
    } else if ((this.stateConfig === STATE_READY) && !this._checkValidityConfig()) {
      this.stateConfig = STATE_INITIAL
    }
  }
  ...
  _checkValidityConfig () {
    ...
    return (
      (
        host?.validity?.valid && host?.value &&
      port?.validity?.valid && port?.value &&
      user?.validity?.valid && user?.value &&
      pw?.validity?.valid && pw?.value &&
      from?.validity?.valid && from?.value &&
      url?.validity?.valid && url?.value
      ) ||
      false
    )
  }
