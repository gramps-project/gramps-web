  ...
  render() {
    return html`
      ...
      >
        ...
        <div
          style="position: absolute; bottom: 40px; left: 10px; z-index: 10; background: white; border-radius: 4px; box-shadow: 0 1px 4px rgba(0,0,0,0.2); padding: 4px 8px; font-size: 14px;"
        >
    ...
    try {
      ...
      this._map.once('styledata', () => {
        ...
        for (const ctrl of controls) {
          if (ctrl && typeof ctrl._updateAttributions === 'function') {
            if (
              ctrl._container &&
              ctrl._container.querySelector('.maplibregl-ctrl-attrib-inner')
            ) {
              ctrl._container.querySelector(
