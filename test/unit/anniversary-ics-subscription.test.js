...
function createSettingsView(version = '3.21.0') {
...
describe('anniversary ICS subscription settings', () => {
  ...
  it('is hidden when only the persistent token API is supported', () => {
    const view = createSettingsView('3.20.1')
    const content = templateText(view.renderContent())

    expect(content).to.contain('Access tokens')
    expect(content).not.to.contain('Anniversary calendar (ICS)')
  })
